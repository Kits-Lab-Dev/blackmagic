...
static usbd_device *l432_usb_driver_init(void)
{
...
void st_usbfs_copy_from_pm(void *buf, const volatile void *vPM, uint16_t len)
{
    ...
    uint8_t odd = len & 1;
    uint16_t word_count = len >> 1;
    uint8_t *dest = (uint8_t *)buf;
    
    // Копируем полные слова
    for (uint16_t i = 0; i < word_count; i++) {
        uint16_t value = PM[i];
        // STM32 USB использует little-endian для слов, но big-endian внутри слов
        dest[i * 2] = value & 0xFF;         // Младший байт первый
        dest[i * 2 + 1] = (value >> 8) & 0xFF; // Старший байт второй
    }
    
    // Обрабатываем последний нечетный байт
    if (odd) {
        uint16_t last_word = PM[word_count];
        dest[word_count * 2] = last_word & 0xFF;
    }
}

static void l432_usb_driver_disconnect(usbd_device *usbd_dev, bool disconnected)
{
	(void)usbd_dev;
...
const struct _usbd_driver l432_usb_driver = {
	.init = l432_usb_driver_init,
	...
	.disconnect = l432_usb_driver_disconnect,
	.poll = st_usbfs_poll,
};